...
window.addEventListener('DOMContentLoaded', async () => {
    try {
        listExpenses(1);
        let allExpenseReport = await axios.get(`http://3.87.224.194:3000/user/getExpenseReports`, { headers: { token: localStorage.getItem('token') } });
...
async function listExpenses(page){
    console.log(page);
    let result = await axios.get(`http://3.87.224.194:3000/expense/getExpenses?page=${page}&rows_per_page=${Number(localStorage.getItem('UserPreference_page_size'))}`, { headers: { token: localStorage.getItem("token") } });
...
addExpenseForm.addEventListener('submit', async (e) => {
    ...
    try {
        const result = await axios.post(`http://3.87.224.194:3000/expense/addExpense`, obj, { headers: { token: localStorage.getItem("token") } });
...
addExpense_dest.addEventListener('click', async (e) => {
    if (e.target.classList.contains('delete')) {
        try {
            let expense = await axios.delete(`http://3.87.224.194:3000/expense/deleteExpense/` + e.target.dataset.id, { headers: { token: localStorage.getItem("token") } });
...
buyPremiumButton.addEventListener('click', async (e) => {
    const response = await axios.get(`http://3.87.224.194:3000/purchase/purchase-premium`, { headers: { token: localStorage.getItem("token") } });
    ...
    var option = {
        ...
        "handler": async function (response1) {
            try {
                await axios.post(`http://3.87.224.194:3000/purchase/update-premium`, { order_id: option.order_id, payment_id: response1.razorpay_payment_id }, { headers: { token: localStorage.getItem("token") } });
...
download_report_button.addEventListener('click', async () => {
    try {
        const response = await axios.get(`http://3.87.224.194:3000/user/download-report`, { headers: { token: localStorage.getItem('token') } });
...
async function showLeaderBoard() {
    ...
    try {
        const leaderBoards_items = await axios.get(`http://3.87.224.194:3000/premium/showLeaderBoard`, { headers: { token: localStorage.getItem('token') } });
